...
## Electricity-system layer — United Kingdom

The optional GENERATE → STORE → CONNECT → TRANSMIT → CONSUME view retains the original postcode search and 45 km radius. The arrows are category labels, not traced supply. No supplier, connection or power-purchase relationship is inferred from proximity.

DESNZ's [Renewable Energy Planning Database](https://www.gov.uk/government/publications/renewable-energy-planning-database-quarterly-extract) supplies solar, wind and battery records under the [explicit OGL dataset licence](https://www.data.gov.uk/dataset/a5b0ed13-c960-49ce-b1f6-3a6bbe0db1b7/repd). The collector discovers the current quarterly CSV from the official publication page. Each record retains its reference, reported development status, record-update date when available, source extract and approximate coordinate. British National Grid coordinates are transformed to WGS84, including the extract's Northern Ireland points; missing/invalid coordinates and unrecognised countries are counted as omissions. REPD tracks projects above 150 kW (1 MW before 2021), so small, old and unreported projects are not comprehensive. A fresh retrieval is not fresh independent verification.

OpenStreetMap contributes nuclear-source sites and mapped 110/132/220/275/400 kV line or cable segments, plus tagged construction/proposed routes with voltage data. Extraction uses each UK nation's ISO3166-2 boundary separately and rejects empty or partial responses. OSM geometry and lifecycle tags are discovery evidence: absence of a lifecycle tag means status unknown, never an assertion that a route is operational. High-voltage distribution is included. Individual line segments are not separate projects. Offshore and untagged infrastructure can be missing. The radius filter uses the nearest point along route geometry rather than its centre.

Substations and data centres reuse the existing postcode-filtered datasets. The substation dataset covers mapped 132 kV+ infrastructure; lower-voltage substations, including Northern Ireland's 110 kV network, are outside that existing layer. Other major industrial demand is not comprehensively inventoried. OSM records carry individual source links and ODbL attribution. REPD MW fields are quoted with their source interpretation and are never added to data-centre demand or presented as battery energy in MWh.

Stage filters group the source's statuses into operational, construction, proposed/awaiting construction, closed, inactive and unknown. The original status remains visible. Records can overlap: different sources, applications, phases or line segments may represent the same site. Counts are source records, not unique projects; capacities are not aggregated. No inferred connecting edges or guessed corridors are drawn.

`scripts/update_electricity.py` downloads all four nations before publishing `electricity.json` and immutable regional files under `electricity/`. The browser requests only files whose geometry bounds intersect the search area, caches them for the session, and ignores results from superseded searches. Failed regional fetches visibly mark results incomplete and can be retried by toggling the view. The weekly workflow validates the snapshot, commits it and explicitly requests a Pages build on main. A failed collection leaves the last complete snapshot intact. The UI shows retrieval dates and flags snapshots older than two weeks; underlying quarterly/community sources can be older.

Kent's manually reviewed Dungeness B closure and Sea Link context remain in `sources/electricity-context.json`, retaining their own review dates. Dungeness is historical generation. Sea Link's Minster point is broad area context, not an exact converter site or invented route; its 2 GW figure is proposed transfer capacity. GOV.UK and Ofgem Crown material retains OGL attribution. Council and National Grid links remain reading leads and are not a licence to import their project boundaries or a claim of data-centre supply.

Validation: `python scripts/check_electricity_data.py` checks complete regional files, geometry bounds, source/licence fields, unique record IDs and coverage in all four nations. `node scripts/check_electricity.cjs` checks regional loading, search races, retries, stages, crossing routes, source escaping and original-map restoration, alongside the existing frontend/navigation/open-data checks.
